...
io.on('message-client', function(msg){
    io.emit('message', msg);
})
...
var live_stream_chat_wopermittion = [];
...
io.on('connection', (socket) => {
  ...
  io.emit('live stream chat wo/permittion all messages', live_stream_chat_wopermittion);
  socket.on('live stream chat wo/permittion', msg => {
    io.emit('live stream chat wo/permittion', msg);
    live_stream_chat_wopermittion.push(msg);
  });

  io.emit('live stream all messages', live_stream_chat);
  socket.on('live stream chat', msg => {
    live_stream_chat.push(msg);
    for(i=0;i<live_stream_chat_wopermittion.length;i++)
    {
      var id =chat_infos_search(live_stream_chat_wopermittion[i],"id",msg.id);
      var name = chat_infos_search(live_stream_chat_wopermittion[i],"name",msg.name);
      var message =chat_infos_search(live_stream_chat_wopermittion[i],"message",msg.message);
      if(id != -1 && name != -1 && message != -1)
      {
        live_stream_chat_wopermittion[i].permission = 1;
        console.log(live_stream_chat_wopermittion[i]);
      }
...
setInterval(() => {
  ...
  chat_file_backup('./live_stream_messages/live_stream_chat_all_messages.json',live_stream_chat_wopermittion);
  live_stream_chat_wopermittion = [];

  chat_file_backup('./info_desk_messages/common_chat.json',common_chat);
  common_chat = [];
